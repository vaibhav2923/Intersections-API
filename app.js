...
// Auth middleware - For now token is test123 for test
...
app.post('/find-intersections', (req, res) => {
    try {
        ...
        // Convert the linestring and lines to turf geometries
        const linestringGeometry = turf.lineString(linestring.coordinates);
        const lineGeometries = lines.map((line) => line.geometry);

        // Find intersections using turf
        const intersections = turf.lineIntersect(linestringGeometry, turf.featureCollection(lineGeometries));

        // Check for intersections
        if (intersections.features.length === 0) {
            return res.json([]); // No intersections
        }

        // Map intersecting lines and points to their IDs
        const intersectingLines = intersections.features.map((intersection) => {
            const { id } = lines.find((line) => turf.booleanEqual(line.geometry, intersection));
